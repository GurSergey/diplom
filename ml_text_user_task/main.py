...
while 1:
    ...
    cursor = conn.cursor()
    cursor.execute("""SELECT id, model_id, filename FROM queue_task_user_file    
    ...
    if row != None:
        ...
        with open('../text_user_file/'+str(row['filename'])+'.csv', 'w', encoding='UTF-8', newline='') as csv_file:
            task = open('../text_user_file/'+str(row['filename']), "r")
        ...
        conn = psycopg2.connect(dbname='diplom', user='user', 
                                password='secret', host='db', port = 5432)
        cursor = conn.cursor()
        sql_update_query = """UPDATE queue_task_user_file SET completed_task = TRUE,
        in_work = FALSE WHERE id = %s"""
        cursor.execute(sql_update_query, [row['id']])
        conn.commit()
        cursor.close
        conn.close
        print("Completed task user file id " + str(row['id']))
    else: 
        cursor.close
        conn.close
        print("No tasks")
